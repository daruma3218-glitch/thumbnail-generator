...
- **【調整指示】**: スライダーによる数値調整。+1=やや、+2=かなり、-1=やや、-2=かなり。
  - 文字サイズ・フォント関連 → [TEXT PLACEMENT] セクションに反映
  - 明るさ・色味・コントラスト関連 → [COLOR SCHEME] セクションに反映
  - インパクト・構図関連 → [COMPOSITION] セクション（+ 必要に応じて全セクション）に反映
  例: 「文字サイズ: かなり大きく(+2)」→ [TEXT PLACEMENT] の size を extra-large, dominant に
  例: 「明るさ: やや暗く(-1)」→ [COLOR SCHEME] の lighting をやや暗め・ムーディーに
  例: 「インパクト: かなり強烈(+2)」→ 全セクションを極端に強調
- **【変更指示】**: 特定の要素を変更する指示。該当するセクションのプロンプト要素を大幅に書き換える。
- **【補足】**: 自由記述。最優先で反映する。
これらのセクションがない場合は、従来通りの自由記述フィードバックとして解釈する。

## promptEnのフォーマット（3セクション構造 — 必ず守ること）
「Generate a YouTube thumbnail image for a Japanese audience.」で始め、以下3セクションを必ず含める:

\`\`\`
Generate a YouTube thumbnail image for a Japanese audience.

[COMPOSITION]
- 人物/被写体の配置、カメラアングル（ローアングル、俯瞰等）
- 前景/背景の関係、レイアウト構成
- 視線誘導、動的な構図（対角線、三分割法等）
- 人物がいる場合は感情表現を誇張する

[COLOR SCHEME]
- メインカラー、アクセントカラー（具体的な色名で指定）
- ライティング（方向、強さ、色温度）
- コントラスト、彩度、グラデーション
- 全体のムード（ドラマチック、ポップ、緊迫感等）

[TEXT PLACEMENT]
- 日本語テキスト内容（3〜5語。必ず日本語）
- フォントスタイル（太字、ゴシック体等）
- 配置位置（上部中央、右下等）
- サイズ（画面の何%を占めるか）
- 縁取り（太い黒縁取り必須）
...
各セクションは具体的・映画的に描写すること。テンプレート的な表現を避け、創造的に書く。
大胆でシンプル。要素を絞り1つの焦点に集中する。
